    ...
    void LateUpdate()
    {
		...
		if (status){
			...
			if(Input.GetMouseButton(1)) {
			 mouseMovement = (Input.mousePosition - (new Vector3(Screen.width, Screen.height, 0) / 2.0f)) * 1;
			} else {
				Vector3 mid = new Vector3(Screen.width/2,Screen.height/2,0);
				mouseMovement =  (mid - (new Vector3(Screen.width, Screen.height, 0) / 2.0f)) * 1;
			}
